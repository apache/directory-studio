...
/**
 * This class implements the create connection action for an ApacheDS 1.5.5 server.
 *
...
public class CreateConnectionAction implements IObjectActionDelegate
{
    ...
    public void run( IAction action )
    {
        if ( view != null )
        {
            ...
            if ( ( !selection.isEmpty() ) && ( selection.size() == 1 ) )
            {
                ...
                ServerConfigurationV155 serverConfiguration = null;
                try
                {
                    serverConfiguration = ApacheDS155LdapServerAdapter.getServerConfiguration( server );
                }
                catch ( Exception e )
                {
                    String message = Messages.getString( "CreateConnectionAction.UnableReadServerConfiguration" ) //$NON-NLS-1$
     ...
     */
    private void createConnection( LdapServer server, ServerConfigurationV155 configuration )
    {
